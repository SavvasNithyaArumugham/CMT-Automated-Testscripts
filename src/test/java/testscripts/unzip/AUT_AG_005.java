...
public class AUT_AG_005 extends TestCase {
	...
	@Test
	public void unzip_05() {
		testParameters
				.setCurrentTestDescription("Verify user can Unzip a folder with destination folder in the Other site selecting Unzip to option under document actions");
	...
	public void executeTest() {
		...
		myFiles.openCreatedFolder(zippedFolderName);
		if(docLibPg.isFileIsAvailable(fileName)){
			report.updateTestLog("Verify the Unzipped content", "Unzipped content verified successfully"
					+ "<br /><b>File Name : </b>" + fileName, Status.PASS);
		}else{
			report.updateTestLog("Verify the Unzipped content", "Unzipped content not available"
