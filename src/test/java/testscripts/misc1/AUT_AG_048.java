...
public class AUT_AG_048 extends TestCase
{
	...
	@Test
	public void MISC1_039()
	{
		testParameters.setCurrentTestDescription("Verify that user is able to generate catalog for the Xml files");
	...
	public void executeTest()
	{
		...
		String moreSettingsOpt = dataTable.getData("Document_Details", "DocumentActionName");
		String catlogueXmlName = dataTable.getData("Document_Details", "Title");
		...
		sitesPage.enterIntoDocumentLibrary();
		myFiles.openFolder(folderName1);
		sitesPage.clickOnMoreSetting(XmlfileName);
		docLibPg.commonMethodForClickOnMoreSettingsOption(XmlfileName,
				moreSettingsOpt);
		sitesPage.enterIntoDocumentLibrary();
		myFiles.openFolder(folderName1);
		if(docLibPg.checkFileIsAvailable(catlogueXmlName)){
			report.updateTestLog(
					"Verify Catalogue generated",
					"Catalogue generated successfully",
					Status.PASS);
		}else{
			report.updateTestLog(
					"Verify Catalogue generated",
					"Catalogue is not generated",
					Status.FAIL);
		}
