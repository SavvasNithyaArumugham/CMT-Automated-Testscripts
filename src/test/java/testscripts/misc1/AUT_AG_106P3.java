...
public class AUT_AG_106P3 extends TestCase {
	...
	public void MISC1_021() {
		testParameters.setCurrentTestDescription(
				"ALFDEPLOY-1130_1333_Verify notification is displayed upon uploading a file with different name than the original edited offline file in Alfresco for new version<br>"
						+ "ALFDEPLOY-1130_1333_Verify the name of the file chosen for upload new version is displayed in update file window upon clicking upload new version for edited offline file");
	...
	public void executeTest() {
		...
		String[] diffFiles = dataTable.getData("Document_Details", "DocumentActionName").split(",");
		...
		for (int i = 0; i < diffFiles.length; i++) {
			sitesPage.clickOnMoreSetting(files[0]);
			docLibPg.commonMethodForClickOnMoreSettingsOption(files[0], moreSettingsOptionName);
			docLibPg.uploadNewVersionInDocLibPage(diffFiles[i], filePath, version, comments);
			if (!files[0].equals(diffFiles[i])) {
				// This fails due to defect
				docLibPg.uploadNewnotification("Filename", diffFiles[i], "new");
			} else {
				docLibPg.uploadNewnotificationPresence("Filename", diffFiles[i], "new");
			}
			docLibPg.clickOnCancelBtnInUploadNewVersion();
			sitesPage.enterIntoDocumentLibraryWithoutReport();
		}
