...
public class ALFDEPLOY_1497_TC001 extends TestCase {
	...
	public void COLLECTIONS_049() {
		testParameters
				.setCurrentTestDescription("1.Confirm outgoing child references is created for the course and content object with files available in assets-->Thumbnails folder.");
	...
	public void executeTest() {
		...
		// Check outgoing child references for Content				
		//  login
		...
		// Upload file in Asset->Thumbnail folder
		sitesPage.enterIntoDocumentLibrary();
		myFiles.openCreatedFolder("Assets");
		myFiles.openCreatedFolder("Thumbnails");
		collectionPg.uploadFileInCollectionSite(filePath,fileName1);
		...
		collectionPg.addThumbnailChildReference("Assets","Thumbnails",folderNames[2]);
			...
			if (UIHelper.checkForAnElementbyXpath(driver,childRefFileAvailableCheck))
		{
			...
			report.updateTestLog("Confirm outgoing child references is created for the content object with files available in Asset folder",
		...
		else
		{
			...
			// Check outgoing child references for Course
			...
			if (UIHelper.checkForAnElementbyXpath(driver,childRefFileAvailableCheck1))
		{
			...
			report.updateTestLog("Confirm outgoing child references is created for the course with files available in Asset folder",
					"New outgoing child references is created", Status.PASS);	
		}
		else
		{
