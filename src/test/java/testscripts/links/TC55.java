...
public class TC55 extends TestCase {
	private FunctionalLibrary functionalLibrary;

	@Test
	public void LINKS_055() {
		testParameters
				.setCurrentTestDescription("Verify that user is able to Unlink a Folder linked at multiple location.");
	...
	public void executeTest() {
		...
		String folderName2 = dataTable.getData("MyFiles", "Version");
		...
		for (String name : folder) {
			...
			if (sitesPage.Checkdocument(folderName)) {
				report.updateTestLog("Verify Linked folder available",
						"Linked folder is available", Status.FAIL);
			} else {
				report.updateTestLog("Verify Linked folder available",
						"Linked folder is not available", Status.PASS);
			}
		}
