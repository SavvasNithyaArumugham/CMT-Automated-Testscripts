...
public class AUT_AG_155 extends TestCase {
	...
	@Test
	public void runAUT_AG_AUT_155() {
		testParameters.setCurrentTestDescription("Validate JSON response if user copies folder level upto 20 levels to another watched content folder from one watched content folder");
	...
	public void executeTest() {
		...
		for (int index = 1; index <= 21; index++) {
			...
			answerForURN = CC_API_Helper.getManifestationReturnresponse(token, finalURNVal);
			marklogic.verifygetManifestationResponse(answerForURN);
			
			String finalWorkURNVal = myFiles.getMarkLogicManifestUrl(actualWorkURL);
			new FileUtil().appendTextToFile(finalWorkURNVal, workURNURLStoreFilePath);
			
			answerForWorkURN = CC_API_Helper.getManifestationReturnresponse(token, finalURNVal);
		...
		if (folderNamesList != null && urnURLsList != null
			...
			for (int index = 0; index < markLogicURLsList.size(); index++) {
				...
				adminPg.navigateToNodeBrowserDetailsPg(folderNamesList.get(index));

				answerForURN = CC_API_Helper.getManifestationReturnresponse(token,
						urnURLsList.get(index));
				answerForWorkURN = CC_API_Helper.getManifestationReturnresponse(token,
		...
		// Perform copy to and check validations for watched content dest parent
		// and child folders
		...
		docDetailsPage.clickCopyToDocAction();
		docDetailsPage.selectFolderToCopyInCopyPopUp(firstChildFolderName,
		...
		for (int index = 1; index <= 21; index++) {
			...
			answerForURN2 = CC_API_Helper.getManifestationReturnresponse(token, finalURNVal);
			marklogic.verifygetManifestationResponse(answerForURN2);
			
			String finalWorkURNVal = myFiles.getMarkLogicManifestUrl(actualWorkURL2);
			new FileUtil().appendTextToFile(finalWorkURNVal, workURNURLStoreFilePath);
			
			answerForWorkURN2 = CC_API_Helper.getManifestationReturnresponse(token, finalURNVal);
		...
		if (folderNamesList2 != null && urnURLsList2 != null
			...
			for (int index = 0; index < markLogicURLsList2.size(); index++) {
				...
				adminPg.navigateToNodeBrowserDetailsPg(folderNamesList2.get(index));

				answerForURN2 = CC_API_Helper.getManifestationReturnresponse(token,
						urnURLsList2.get(index));
				answerForWorkURN2 = CC_API_Helper.getManifestationReturnresponse(token,
