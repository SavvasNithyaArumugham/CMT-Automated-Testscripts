...
public class AUT_AG_139 extends TestCase {
	...
	@Test
	public void runAUT_AG_139() {
		testParameters.setCurrentTestDescription("Validate the JSON response for work and manifestation URN generated if user deletes a file from watched content folder");
	...
	public void executeTest() {
		...
		if (sitesPage.checkMoreSettingsOption(folderName, moreSetOptions[0])) {
			...
			ArrayList<ArrayList<String>> finalManifestationResponseAfterDelete = adminPg.verifyJSONResponse("FILE", "manifestation", baseURN, UIManifestationTagMap, mapManifestAfterDelete);
			ArrayList<ArrayList<String>> finalWorkResponseAfterDelete = adminPg.verifyJSONResponse("FILE", "work", workbaseURN, UIWorkTagMap, mapWorkAfterDelete);
			
			if(finalManifestationResponseAfterDelete.get(1).size()==0){
				report.updateTestLog("Verify the Manifestation JSON response After delete a file",
			...
			if(finalWorkResponseAfterDelete.get(1).size()==0){
				report.updateTestLog("Verify the Work JSON response After delete a file",
						"Values verified successfully"
						+"</br> <b> Verified Tag Details : </b>" +finalWorkResponseAfterDelete.get(0).toString(), Status.PASS);
			}else{
				report.updateTestLog("Verify the JSON response After delete a file",
						"Values not matched "
