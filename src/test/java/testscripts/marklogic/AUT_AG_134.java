...
public class AUT_AG_134 extends TestCase {
	...
	@Test
	public void runAUT_AG_AUT_134() {
		testParameters.setCurrentTestDescription("Validate the JSON response for work and manifestation URN generated when user links a folder from watched content folder to watched content folder");
	...
	public void executeTest() {
		...
		if (sitesPage.checkMoreSettingsOption(folderName2, moreSetOptions[0])) {
			report.updateTestLog("Verify '" + moreSetOptions[0]
					+ "' aspect for empty folder under 'More' option",
					"Verified Successfully", Status.PASS);
		...
		sitesPage.commonMethodForClickOnMoreOptionLink(folderName1,
				moreSetOptions[2]);
		docLibPage.performLinkToOperation(siteNameValue, folderName2);
		
		myFiles.openCreatedFolder(folderName2);
		docLibTestPage.verifyContentCategoryItemInDocLibPag(folderName1, categoryItem);
		sitesPage.clickOnMoreSetting(folderName1);
		sitesPage.clickOnMoreLinkOptions(folderName1, moreSetOptions[1]);
		docDetailsPage.clickAllProperties();
