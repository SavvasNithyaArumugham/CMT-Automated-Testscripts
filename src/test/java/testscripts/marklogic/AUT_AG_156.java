...
public class AUT_AG_156 extends TestCase {
	...
	@Test
	public void runAUT_AG_AUT_156() {
		testParameters
				.setCurrentTestDescription("Validate the JSON response for work and manifestation URN generated if user deletes folder level upto 20 levels under watched content folder");
	...
	public void executeTest() {
		...
		String dataFolderName = "", parentdataFolderChildFodler = "";
		...
		if (folderDetails.contains(",")) {
			...
			parentFolderName = folderName + "WC";

			myFiles.commonMethodFordeleteFolder(parentFolderName);

			myFiles.commonMethodForCreateFolder(parentFolderName, folderTitle,
					folderDescription);
		...
		// Perform Delete 20 level subfolder and check validations for watched content dest parent
		...
		String folderNameVal = parentFolderName;
		sitesPage.clickOnMoreSetting(folderNameVal);
		...
		adminPg.navigateToNodeBrowserDetailsPg(folderNameVal);
