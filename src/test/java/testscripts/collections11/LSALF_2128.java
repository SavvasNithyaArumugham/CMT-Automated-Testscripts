...
public class LSALF_2128 extends TestCase {

	private FunctionalLibrary functionalLibrary;

	@Test
	public void COLLECTIONS_01_02() {
		testParameters.setCurrentTestDescription("Verify on importing a print plan CSV with Dynamic Content Type left blank, the error report is generated and attached to the completed course CSV once moved to completed folder.");
	...
	public void executeTest() {
		...
		// Import Original Course
				...
				UIHelper.waitFor(driver);
				// Import empty dynamic content type in the DC object of the csv		
				sitesPage.enterIntoDocumentLibrary();
		// go to Course plan
		myFiles.openCreatedFolder("Data Imports");
		UIHelper.waitFor(driver);
		...
		collectionPg.clickOnMoreSetting(filepathfilename);
		UIHelper.waitFor(driver);
		
		new FileUtil().deleteIfFileExistsInDownloadPath(downloadFilePath,errorfile);	
		collectionPg.commonMethodForClickOnMoreSettingsOption(filepathfilename, "View Error Report");
		UIHelper.waitFor(driver);					
		...
		UIHelper.waitFor(driver);
		UIHelper.waitFor(driver);
		collectionPg.errormessagecontains("Dynamic Content type","'Dynamic Content type' must not be BLANK.");
