...
public class LSALF_2123_3CnCSV extends TestCase {
		...
		sitesPage.openSiteFromRecentSites( siteNameValue);
				...
				if (downloadedFile.exists() && !filename2.equalsIgnoreCase("File Not Found")) {
					...
					try {
						...
						if(CSVUtil.verifyDataInCell(downloadedCSVFileANmeWithPath,"Core Texts|Hook and Inspire Texts",4,98))
						{
						report.updateTestLog("Confirm Library property value is exporting properly ",
								"Column CU (Library):"+CSVUtil.readDataInCell(downloadedCSVFileANmeWithPath,4,98), Status.PASS);
						}else {
							report.updateTestLog("Confirm Library property value is not exporting properly ",
									"Column CU (Library):"+CSVUtil.readDataInCell(downloadedCSVFileANmeWithPath,4,98), Status.FAIL);
						}
					} catch (IOException e) {
