...
public class AUT_AG_092 extends TestCase {
	...
	public void runAUT_AG_AUT_092() {
		testParameters
				.setCurrentTestDescription("Verify the navigation on clicking breadcrumb form view details page for the"
						+"1. Linked folders within shared files"
						+"2. Linked folders from shared files to other site");
	...
	public void executeTest() {
		...
		if (folderDetails.contains(",")) {
			...
			for (int index = 1; index <= 2; index++) {
				if(index==1)
				{
					...
					myFiles.deleteCreatedFolder(firstFolderName);
				}
				else
				{
		...
		docLibPg.performLinkToOperation("Shared File", secondFolderName);
		...
		docLibPg.performUnLinkOperation("Shared", secondFolderName);
