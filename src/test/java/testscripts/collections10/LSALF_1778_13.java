...
public class LSALF_1778_13 extends TestCase {
			...
			for (String csvRow : csvFileRowDataList) {
				...
				for (String listOfObjectsString : filterselection1) {
					for(int i =0;i<splitedRow.length;i++) {
						System.out.println("splitedRow"+splitedRow[i]);
					if (listOfObjectsString.equals(splitedRow[i].replace("\"", ""))) {
						report.updateTestLog(listOfObjectsString + " header is appearing in the csv file ",
								"Status: " + listOfObjectsString + "appeared in the csv file", Status.FAIL);
					} else {
						report.updateTestLog(listOfObjectsString + " header is not appearing in the csv file ",
								"Status: " + listOfObjectsString + " not appeared in the csv file", Status.PASS);
						break;
					}		
				}
				}
