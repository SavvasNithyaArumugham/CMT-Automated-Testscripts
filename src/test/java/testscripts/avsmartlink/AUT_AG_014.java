...
public class AUT_AG_014 extends TestCase {
	...
	public void AVSMARTLINK_008() {
		testParameters.setCurrentTestDescription(
				"1.Verify new smart links are created in folders based on the spreadsheet values on selecting 'Create Multi Smart Link' option for the valid sl_3rd_party.csv file<br>"
						...
						+ "<br>R2.0.2_ALFDEPLOY-6248_Verify if Optimized for use on Mobile radio button for multi smartlink is default to Yes in 3rd Party Interactive Smartlink"
						+ "<br>R2.0.2_ALFDEPLOY-6082_Verify if user is able to change the Optimized for use on Mobile radio button while editing a 3rd party interactive smartlink.");
	...
	public void executeTest() {
		...
		String imgFile = dataTable.getData("MyFiles", "CreateFolder");
		...
		myFiles.uploadFile(filePath, imgFile);
		myFilesTestObj.verifyUploadedFile(imgFile, "");
		...
		if (downloadedFile.exists() && !csvName.equalsIgnoreCase("File Not Found")) {
			...
			for (String csvRow : csvFileRowDataList) {
				...
				if (smartLinkList.contains(splitedRow[0].replace("\"", ""))
						&& splitedRow[10].replace("\"", "").equals("Success")
						&& splitedRow[8].replace("\"", "").equals("Yes")) {
		...
		// check post multi smartlink,optimized radio button defaulted to yes &
		// change to no in edit smartlink
		AlfrescoCollectionsPage collectionPg = new AlfrescoCollectionsPage(scriptHelper);
		collectionPg.clickOnMoreSetting(edittitle);
		collectionPg.commonMethodForClickOnMoreSettingsOption(edittitle, "Edit SmartLink");
		if (avsmart.optimizedSelection()) {
			report.updateTestLog(
					"Verify Optimized for use on Mobile radio button in UI is default to Yes in multi smartlink",
					"Optimized for use on Mobile radio button in UI is defaulted to Yes in multi smartlink",
					Status.PASS);
		} else {
			report.updateTestLog(
					"Verify Optimized for use on Mobile radio button in UI is default to Yes in multi smartlink",
					"Optimized for use on Mobile radio button in UI is not defaulted to Yes in multi smartlink",
					Status.FAIL);
		}
		avsmart.selectimagefromSelect("thirdPartyInteractiveLink", siteName, imgFile);
		avsmart.EditSmartlinkFilePathValidation("thirdPartyInteractiveLink", edittitle, siteName, imgFile);
		avsmart.toggleOptimizedSelection("No");
		avsmart.cancelButtonOnEditSmartlink("thirdPartyInteractiveLink", edittitle);
