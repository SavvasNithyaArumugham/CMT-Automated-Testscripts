...
public class AUT_AG_041P1 extends TestCase {
	...
	public void AVSMARTLINK_008() {
		testParameters.setCurrentTestDescription(
				...
				"Verify Co-ordinator/Collaborator/Contributor is getting Edit smartlink  option displayed for External Web site smartlink object which is created by manager and manager is able to edit<br>"
						+ "R2.0.2_ALFDEPLOY-5984_Verify user is able to create smartlink successfully on entering backslash in Image Alt Text, Caption Text, Copyright or Credit Text fields for External Website link.");
	...
	public void executeTest() {
		...
		try {
			...
			avsmart.entersmarttypedata(Name, title, extURLLink, "Image", "Caption", "credit", "",
			...
			avsmart.entersmarttypedata(Name, edittitle, extURLLink, "Images", "Captions", "credits", "",
					System.getProperty("user.dir") + filePath + fileName);
			...
			myFiles.createcontenttype(type);
			avsmart.clickSmartLinkType(Type, Name);
			avsmart.entersmarttypedata(Name, "ExternalWebsiteSLBackslash", extURLLink, "\\\"", "\"\"", "\\b", "",
					System.getProperty("user.dir") + filePath + fileName);

			avsmart.submitbutton(Name, "ExternalWebsiteSLBackslash");
			sitesPage.clickOnMoreSetting("ExternalWebsiteSLBackslash");
			docLibPg.commonMethodForClickOnMoreSettingsOption("ExternalWebsiteSLBackslash", editSL);
			avsmart.EditSmartlinkFilePathValidation(Name, "ExternalWebsiteSLBackslash", siteName, fileName);
			avsmart.cancelButtonOnEditSmartlink(Name, "ExternalWebsiteSLBackslash");

			myFilesTestObj.verifyUploadedFile("ExternalWebsiteSLBackslash", "");
		} catch (Exception e) {
			e.printStackTrace();
			report.updateTestLog("AUT_AG_030 status:", "<br>AUT_AG_024 Testcase is Failed", Status.DONE);
