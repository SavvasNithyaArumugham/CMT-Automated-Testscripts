...
public class AUT_AG_075 extends TestCase {
	...
	public void AVSMARTLINK_008() {
		testParameters.setCurrentTestDescription(
				"ALFDEPLOY-3633_ALFDEVEL-1176_Verify the created Smart Link folders for 3rd party interactive link have Smart Link object appropriately.<br>"
		...
		if (downloadedFile.exists() && !csvName.equalsIgnoreCase("File Not Found")) {
			...
			for (String csvRow : csvFileRowDataList) {
				...
				if (smartLinkList.contains(splitedRow[0].replace("\"", ""))
						&& splitedRow[9].replace("\"", "").equals("Success")) {
					report.updateTestLog("SmartLink: " + splitedRow[0], "Status: " + splitedRow[9], Status.PASS);
				}
			}
		} else {
