...
public class AUT_AG_073 extends TestCase {
	...
	public void AVSMARTLINK_008() {
		testParameters.setCurrentTestDescription(
				"ALFDEPLOY-3633_ALFDEVEL-1176_Verify the created Smart Link folders for Audio link have Smart Link object appropriately.<br>"
						...
						+ "ALFDEPLOY-3633_ALFDEVEL-1199_Verify smart link object not created when Image preview field provided with a site URL path from diff env or repo and check the status column in the csv updated with Error - Image is not available.Reverted");
	...
	public void executeTest() {
		...
		if (!docLibPg.isFileIsAvailable("AudioSmartLink5")) {
			report.updateTestLog("Verify error row is not created",
					"File: " + "AudioSmartLink5" + " error row is not created/available", Status.PASS);

		} else {
			report.updateTestLog("Verify error row is not created",
					"File: " + "AudioSmartLink5" + " error row is created/available", Status.FAIL);
		...
		if (downloadedFile.exists() && !csvName.equalsIgnoreCase("File Not Found")) {
			...
			for (String csvRow : csvFileRowDataList) {
				...
				if ("AudioSmartLink5".equals(splitedRow[0].replace("\"", ""))
