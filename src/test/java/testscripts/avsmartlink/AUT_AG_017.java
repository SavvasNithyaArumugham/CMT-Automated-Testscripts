...
public class AUT_AG_017 extends TestCase {
	...
	public void AVSMARTLINK_008() {
		testParameters.setCurrentTestDescription(
				"1.Verify new smart links are created in folders based on the spreadsheet values on selecting 'Create Multi Smart Link' option for the valid sl_pdf.csv file<br>"
				+"2. Verify the created Smart Link folders for PDF link have Smart Link object appropriately.<br>"
				+"3. Validate the smart link is updated correctly when Title,external URL,JSON string fields are edited via edit properties for PDF link.");
		...
		if (downloadedFile.exists() && !csvName.equalsIgnoreCase("File Not Found")) {
			...
			for (String csvRow : csvFileRowDataList) {
				...
				if (smartLinkList.contains(splitedRow[0].replace("\"", ""))
						&& splitedRow[4].replace("\"", "").equals("Success")) {
					report.updateTestLog("SmartLink: " + splitedRow[0], "Status: " + splitedRow[4], Status.PASS);
				}
			}
		} else {
