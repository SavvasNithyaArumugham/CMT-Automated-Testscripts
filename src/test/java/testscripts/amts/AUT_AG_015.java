...
public class AUT_AG_015 extends TestCase {
	...
	@Test
	public void AUT_AG_015() {
		
		testParameters
		.setCurrentTestDescription("ALFDEPLOY-5071_Verify the presence of Optimized folder and folder applied with optimization rule in the document library of the selected site <br>"+
				"ALFDEPLOY-5071_Verify the lifecycle states upon attaching lifecycle name-Image Optimization for image files <br>"+
				"ALFDEPLOY-5071_Verify the lifecycle states when user is in default or first state of lifecycle -Image optimization for image file <br>"+
				"ALFDEPLOY-5071_Verify optimized file is created in Optimized folder upon promoting the lifecycle state of an image to Optimize for JPG from Ready for Optimization<br>"+
				"ALFDEPLOY-5071_Verify optimized file is created in Optimized folder upon promoting the lifecycle state of an image having keyword to Optimize for JPG from Ready for Optimization");
	...
	public void executeTest() {
		...
		{
		myFiles.uploadFileInMyFilesPage(filePath, fileName[x]);
		
		//Perform attach lifecycle via more option
		sitesPage.clickOnMoreSetting(fileName[x]);
		sitePgTestObj.verifyMoreSettingsOptionForFileOrFolderItem(fileName[x], moreSettingsOption);
		docLibPge.commonMethodForClickOnMoreSettingsOption(fileName[x], moreSettingsOption);
		docDetailsPg.lifeCycleDropdown(lifecycleAction);
		docDetailsPg.changeLifeCycleSate(attachLifeCycleDropdownVal);
		sitesPage.documentdetails(fileName[x]);
		
		String ActualLifecycle = appSearchPg.getMetadata(fileName[x],
				"Lifecycle Name:");
		...
		String defaultState = appSearchPg.getMetadata(fileName[x],
				"State:");
		...
		String ActualLifecycleState = appSearchPg.getMetadata(fileName[x],
				"State:");
		...
		sitesPage.documentdetails(optimizedFolder);	
		if (docLibPge.isFileIsAvailable(finalFileimg[x])) {
			...
			sitesPage.documentdetails(finalFileimg[x]);
