...
public class AUT_AG_055 extends TestCase {
	...
	public void AMTS_075() {
		testParameters
				.setCurrentTestDescription("Verify the user is not able to see add button for video create profile");
	...
	public void executeTest() {
		...
		if (mediaTransPage.clickOnAddImageProfBtn()) {
			...
			if (mediaTransPage.isAddBtnAvailableInTransfromPg()) {
				report.updateTestLog(
						"Verify [ADD] button is available in Video transfrom page",
						"[ADD] button is displayed", Status.FAIL);

			} else {
				report.updateTestLog(
						"Verify [ADD] button is available in Video transfrom page",
						"[ADD] button is not displayed.", Status.PASS);
			}
		...
		fileNme = fileNamePart[0] + "_" + subAsstCode + "_" + "."
				+ fileNamePart[1];
