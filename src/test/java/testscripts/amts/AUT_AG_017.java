...
public class AUT_AG_017 extends TestCase {
	...
	@Test
	public void AUT_AG_017() {
		
		testParameters
		.setCurrentTestDescription("ALFDEPLOY-5071_Verify the presence of lifecycle name-Image Optimization upon clicking Attach lifecycle for image file which is present inside the rule applied folder from More option or file preview page or selected items of Document library<br>"+
		"ALFDEPLOY-5071_Verify the presence of lifecycle name-Image Optimization upon clicking Attach lifecycle for image file which is present outside the rule applied folder from More option or file preview page or selected items of Document library");
	...
	public void executeTest() {
		...
		for(int j=0;j<folderNonOptimized.length;j++)
		{
			sitesPage.enterIntoDocumentLibrary();
			if(sitesPage.documentAvailable(folderNonOptimized[j]))
			{
				sitesPage.documentdetails(folderNonOptimized[j]);
			}
			else
			{
	...
	for(int i=0;i<fileName.length;i++)
	{
		...
		docDetailsPg.backToFolderOrDocumentPage(folderNonOptimized[j]);
	}
	}
