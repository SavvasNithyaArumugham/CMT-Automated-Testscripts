...
public class AUT_AG_007 extends TestCase {
	...
	@Test
	public void AMTS_084() {
		testParameters
				.setCurrentTestDescription("Verify that macro code is replaced with subasset code in transformed file when filename based transformation is done for folder containing video file with filename(filename_macrocode_) <br>" +
		"ALFDEPLOY-4427_Verify the target output filename does not cut off period present in source file when filebased video transformation is applied for folder containing video files with period<br>"+
						"ALFDEPLOY-4427_Verify the target output filename does not cut off period present in source file when image transformation is applied for folder containing image files with multiple period");
	...
	public void executeTest() {
		...
		String reName = fileName.replace(".", ".._"+macCode+"_.");
		...
		mediaTransPage.applyTransformationToTargetFolder(profName,
				fileName, watchFolderName, "", sourceSiteName);
	...
	//	mediaTransPage.applyTransformationByFileBased(reName, preTxt);
		...
		String finalFileName = fileName.replace(".3gp", ".._"+subAsstCode+"."+imageType);
