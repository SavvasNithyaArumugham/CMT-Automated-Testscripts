...
public class AUT_AG_015P2 extends TestCase {

	private FunctionalLibrary functionalLibrary;

	@Test
	public void AUT_AG_015P2() {
		
		testParameters
		.setCurrentTestDescription("ALFDEPLOY-5071_Verify optimized file is created in Optimized folder for updated source file upon promoting the lifecycle state of the image to Optimize for JPG from Ready for Optimization");
	...
	public void executeTest() {
		...
		String action = dataTable.getData("Document_Details", "DocPropertyValues");
		String versionDetails = dataTable.getData("Document_Details", "Version");
		String uploadVersionDetails = dataTable.getData("Document_Details", "AspectName");
		String comments = dataTable.getData("Document_Details", "Comments");
		String sourceVersion = dataTable.getData("Document_Details", "ExtractTo");
		...
		docLibPge.uploadNewVersionInDocLibPage(fileName,
				filePath, uploadVersionDetails, comments);		
		docLibPge.uploadNewVersionButton();
		sitesPage.documentdetails(fileName);
		mediaTransformPage.checkVersionOfTransformedFile(sourceVersion);
		...
		//Promote Lifecycle via document details page post upload new version
		docDetailsPg.commonMethodForPerformDocAction(lifecycleAction);
		docDetailsPg.changeLifeCycleSate(promotOrDemoteOrDetachStateDropdownValue);
