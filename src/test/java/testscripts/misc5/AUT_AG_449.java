...
public class AUT_AG_449 extends TestCase
{
	private FunctionalLibrary functionalLibrary;
	
	@Test
	public void copyToRepoSharedAndLinkedSubFolderCheck()
	{
		testParameters.setCurrentTestDescription("1.ALFDEPLOY-5929_Verify linked shared sub folder becomes unshared and unlinked in target location(shared folder) upon copying to different repository via copy to repo"
				   );
	...
	public void executeTest()
	{
		...
		if(myFiles.isFileOrFolderDisplayed(toVerifyFolderName)) {
			myFiles.deleteCreatedFolder(toVerifyFolderName);
		}
	   ...
	   if(shareboxPg.checkShareIconAvailable(folderName[0])){
			...
			sitesPage.clickOnMoreSetting(toVerifyFolderName);
			docLibPage.commonMethodForClickOnMoreSettingsOption(toVerifyFolderName, moreSettingsOption[1]);
			docDetailsPageObj.selectAFolderInLinkPopUp(siteNameValue,folderName[1]);
			docDetailsPageObj.clickLinkBtnInLinkPopUp();
		
		
			if(shareboxPg.checkShareIconAvailable(toVerifyFolderName) && docLibPage.isLinkIconAvailable(toVerifyFolderName) && !repositryName.isEmpty()) {
				sitesPage.clickOnMoreSetting(toVerifyFolderName);
				sitesPage.commonMethodForClickOnMoreOptionLink(toVerifyFolderName, moreSettingsOption[2]);
				...
				homePageObj.navigateToSharedFilesTab();
				if (myFiles.isFileOrFolderDisplayed(toVerifyFolderName) && !shareboxPg.checkShareIconAvailable(toVerifyFolderName) && !docLibPage.isLinkIconAvailable(toVerifyFolderName)) {
					report.updateTestLog("Verify  shared and linked sub folder become unshared and unlinked after copy to repo",
						"Shared and linked sub folder become unshared and unlinked after copy to repo successfull", Status.PASS);
				} else {
					report.updateTestLog("Verify shared folder become unshared and unlinked after copy to repo",
						"Shared and linked sub folder become unshared and unlinked after copy to repo failed", Status.FAIL);
				}
		  
			
			} else {
				report.updateTestLog("Verify shared folder become unshared and unlinked after copy to repo",
					"Pre condition of sub folder share and link itself failed", Status.FAIL);
			}
	}else {
		report.updateTestLog("Verify shared folder become unshared and unlinked after copy to repo",
