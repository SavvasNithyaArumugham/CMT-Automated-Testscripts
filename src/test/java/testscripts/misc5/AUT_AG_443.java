...
public class AUT_AG_443 extends TestCase
{
	private FunctionalLibrary functionalLibrary;
	
	@Test
	public void copyToRepoSharedFolderCheck()
	{
		testParameters.setCurrentTestDescription("1.ALFDEPLOY-5929_Verify shared folder becomes unshared in target location upon copying to different repository via copy to repo"
				   );
	...
	public void executeTest()
	{
		...
		if(shareboxPg.checkShareIconAvailable(folderName) && !repositryName.isEmpty()) {
			sitesPage.clickOnMoreSetting(folderName);
			...
			if (myFiles.isFileOrFolderDisplayed(folderName) && !shareboxPg.checkShareIconAvailable(folderName)) {
				report.updateTestLog("Verify shared folder become unshared after copy to repo",
						"Shared folder become unshared after copy to repo successfull", Status.PASS);
			} else {
				report.updateTestLog("Verify shared folder become unshared after copy to repo",
						"Shared folder become unshared after copy to repo failed", Status.FAIL);
			}
