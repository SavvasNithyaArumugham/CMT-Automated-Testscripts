...
public class AUT_AG_446 extends TestCase
{
	private FunctionalLibrary functionalLibrary;
	
	@Test
	public void copyToRepoMulitpleSharedSubFolderCheck()
	{
		testParameters.setCurrentTestDescription("1.ALFDEPLOY-5929_Verify shared folder containing multiple shared sub folders become unshared in target location(Shared folder) upon copying to different repository via copy to repo"
				   );
	...
	public void executeTest()
	{
		...
		if(myFiles.isFileOrFolderDisplayed(folderName)) {
			myFiles.deleteCreatedFolder(folderName);
		}
	   ...
	   if(shareboxPg.checkShareIconAvailable(folderName)){
			...
			if(shareboxPg.checkShareIconAvailable(multipleFolderName[0]) &&  shareboxPg.checkShareIconAvailable(multipleFolderName[1]) && !repositryName.isEmpty()) {
				sitesPage.enterIntoDocumentLibrary();
				sitesPage.clickOnMoreSetting(folderName);
				sitesPage.commonMethodForClickOnMoreOptionLink(folderName, moreSettingsOption[1]);
				...
				homePageObj.navigateToSharedFilesTab();
				if (myFiles.isFileOrFolderDisplayed(folderName) && !shareboxPg.checkShareIconAvailable(folderName)) {
					myFiles.openCreatedFolder(folderName);
					if(myFiles.isFileOrFolderDisplayed(multipleFolderName[0]) && myFiles.isFileOrFolderDisplayed(multipleFolderName[1]) && !shareboxPg.checkShareIconAvailable(multipleFolderName[0]) &&  !shareboxPg.checkShareIconAvailable(multipleFolderName[1])) {
						report.updateTestLog("Verify shared folder become unshared after copy to repo",
								"Shared parent folder and mulitple sub foolder become unshared after copy to repo successfull", Status.PASS);
					}
					else {
						report.updateTestLog("Verify shared folder become unshared after copy to repo",
								"Shared parent folder and mulitple sub foolder become unshared after copy to repo failed", Status.FAIL);
					}
						
						
				} else {
					report.updateTestLog("Verify shared folder become unshared after copy to repo",
						"Shared parent folder itself unshared after copy to repo", Status.FAIL);
				}
