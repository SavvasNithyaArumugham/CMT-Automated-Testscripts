 ...
 * @author Cognizant
 */
public class AUT_AG_380 extends TestCase {
	...
	public void executeTest() {
	    ...
	    if(taskPage.checkCreatedOrAssignedTask(userName +" - "+fileName)){
	    	report.updateTestLog("Verify Content fulfilment workflow", 
	    			"Content fulfilment workflow in completed status.", Status.PASS);
	    }else{
	    	
	    	
	    	report.updateTestLog("Verify Content fulfilment workflow",
	    			"Content fulfilment workflow Not in completed state. Verified failed", Status.FAIL);
