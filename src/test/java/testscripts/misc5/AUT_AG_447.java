...
public class AUT_AG_447 extends TestCase
{
	private FunctionalLibrary functionalLibrary;
	
	@Test
	public void copyToRepoLinkedSharedFolderCheck()
	{
		testParameters.setCurrentTestDescription("1.ALFDEPLOY-5929_Verify linked shared folder becomes unshared and unlinked in target location(shared folder) upon copying to different repository via copy to repo"
				   );
	...
	public void executeTest()
	{
		...
		if(shareboxPg.checkShareIconAvailable(folderName[0]) && docLibPage.isLinkIconAvailable(folderName[0])  && !repositryName.isEmpty()) {
			sitesPage.clickOnMoreSetting(folderName[0]);
			...
			if (myFiles.isFileOrFolderDisplayed(folderName[0]) && !shareboxPg.checkShareIconAvailable(folderName[0]) && !docLibPage.isLinkIconAvailable(folderName[0])) {
				report.updateTestLog("Verify linked shared folder becomes unshared and unlinked",
						"Shared folder become unshared and unlinked after copy to repo successfull", Status.PASS);
			} else {
				report.updateTestLog("Verify linked shared folder becomes unshared and unlinked",
						"Shared folder become unshared and unlinked after copy to repo failed", Status.FAIL);
			}
