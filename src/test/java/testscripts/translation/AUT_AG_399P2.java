...
public class AUT_AG_399P2 extends TestCase {
	...
	@Test
	public void translation_3465_07() {
		testParameters.setCurrentTestDescription("ALFDEPLOY-3465_Verify a German translation for a newly translated"
				+ "text for External Sharebox UI - Part 2");
	...
	public void executeTest() {
		...
		shareboxPg.clickLanguageInExternalSharebox("German");
		//Need to write few more code once gmail mail not receiving issue has been fixed
		...
		for (String labelItem : inputLableText) {
			...
			//actualExternalShareboxTextItems = shareboxPgTeSst.getExcludeShareboxTranslationText(labelItem);
		}
		...
		if (UIHelper.compareTwoSimilarLists(expectedExternalShareboxTextItems, actualExternalShareboxTextItems)) {
			report.updateTestLog("Verify Label text in Externalsharebox page",
					"<b> " + expectedExternalShareboxTextItems + " </b>" +"actual : "+actualExternalShareboxTextItems+ "</b> is present", Status.PASS);
		} else {
			report.updateTestLog("Verify Label text in Externalsharebox page",
					"<b> " + expectedExternalShareboxTextItems + " </b>" +"actual : "+actualExternalShareboxTextItems+ "</b> is not present", Status.FAIL);
