 ...
 * @author Cognizant
 */
public class AUT_AG_401P2 extends TestCase {

	@Test
	public void translation_3465_08() {
		testParameters.setCurrentTestDescription("ALFDEPLOY-3465_Verify a German translation for all a external "
				+ "sharebox UI when a language is switched from Swedish to German - Part 2");
	...
	public void executeTest() {
		...
		ArrayList<String> actualExternalShareboxSwedishTextItems = new ArrayList<String>();
		...
		shareboxPg.clickLanguageInExternalSharebox("Swedish");
		...
		actualExternalShareboxSwedishTextItems.addAll(tempExternalShareboxTextItems);

		expectedExternalShareboxTextEnglishItems = myFilesTest.getUploadedFileDetails(shareboxEnglishLabels);

		if (!actualExternalShareboxSwedishTextItems.contains(expectedExternalShareboxTextEnglishItems)) {
			report.updateTestLog("Verify Labels after switching from English to Swedish",
		...
		// switch to German
		shareboxPg.clickLanguageInExternalSharebox("Tyska");
		...
		if (!(actualExternalShareboxGermanTextItems.contains(actualExternalShareboxSwedishTextItems))) {
			report.updateTestLog("Verify Labels after switching from swedish to German",
					"No Labels are present in French", Status.PASS);
		} else {
			report.updateTestLog("Verify Labels after switching from swedish to German", "Labels are present in French",
					Status.FAIL);
		}
