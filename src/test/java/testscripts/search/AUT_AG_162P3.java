 ...
 * @author Cognizant
 */
public class AUT_AG_162P3 extends TestCase
{
		...
		try {
			...
			if(!UIHelper.checkForAnElementbyXpath(driver, finalChkBoxXpathForSearchResultsItem)){
				...
				report.updateTestLog(
						"Verify consumer cannot perform Download as Zip via selected items in Search Results",
						"Consumer cannot perform Download as Zip via selected items in Search Results",
						Status.PASS);
				
			}else{
				report.updateTestLog(
						"Verify consumer cannot perform Download as Zip via selected items in Search Results",
						"Consumer can perform Download as Zip via selected items in Search Results",
						Status.FAIL);
			}
		} catch (Exception e) {
