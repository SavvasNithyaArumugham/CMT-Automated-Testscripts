 ...
 * @author Moshin
 */
public class AUT_AG_140 extends TestCase {
	private FunctionalLibrary functionalLibrary;

	@Test
	public void search_139() {
		testParameters
				.setCurrentTestDescription("Verify that user is able to save the search item from the search page .(Public)");
	...
	public void executeTest() {
		...
		String Search = dataTable.getData("Search", "SavedSearchName");
		Date date = new Date();
		SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
		String strDate = sdf.format(date);
		
		String savedsearch = Search + "- " + strDate;
		appSearchPg.saveSearchParam(savedsearch);
		
		homePage.navigateToHomePage();
		homePage.searchSavedSearchType(type);
		appSearchPg.selectValFrmDetailedSearches(savedsearch);	
