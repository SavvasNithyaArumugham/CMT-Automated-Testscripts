 ...
 * @author Moshin
 */
public class AUT_AG_117P2 extends TestCase {
	private FunctionalLibrary functionalLibrary;

	@Test
	public void search_116() {
		testParameters
				.setCurrentTestDescription("Verify that  if user executes a search from Saved Searches query, search results is based on user privileges - Part 2");
	...
	public void executeTest() {
		...
		try{
			String searchQryname = new FileUtil()
			.readDataFromFile(appSearchPg.testOutputFilePathSveSrch);	
			System.out.println("from P1"+searchQryname);
			homePage.navigateToHomePage();
		...
		appSearchPg.clickSearch();
		appSearchPgTest.verifyFileNameSearchResults();
		String searchQrynameNew = appSearchPg.saveSearchwithUniqueNme();
		System.out.println("New P2 value"+searchQrynameNew);
		homePage.navigateToHomePage();
		homePage.searchSavedSearchType(type);
		appSearchPg.selectValFrmDetailedSearches(searchQrynameNew);
		//homePage.selectValueFrmDashletList(listXpath, Name);
		UIHelper.waitForPageToLoad(driver);
		UIHelper.waitFor(driver);
		appSearchPg.clickSearch();
